...
public class GameEngine {
    ...
    public void reset() {
        ...
        // Safe Zone Helipad near Administrative Building & Central Plaza
        ...
        // Spawn a small initial wave so the map is not empty at the start
        spawnRandomSurvivorWave(5, 12); // 5–12 people at the beginning
        ...
        statusMessage = "Level: " + difficultyLevel + " | Survivors appear every 30s! | WASD + Arrows + Space/R";
    }
    ...
    private void spawnRandomSurvivorWave(int minCount, int maxCount) {
        ...
        int spawned = 0;
        int attempts = 0;
        final int maxAttempts = count * 30; // safety limit

        while (spawned < count && attempts < maxAttempts) {
            attempts++;

            // Random position anywhere on the map (with a little margin from edges)
            double x = 80 + random.nextDouble() * (mapWidth - 160);
            double y = 80 + random.nextDouble() * (mapHeight - 160);

            // Reject if inside a solid building (not FIELD)
            ...
            for (Building b : buildings) {
                ...
                if (!"FIELD".equals(b.getType()) &&
                    x >= b.getX() && x <= b.getX() + b.getWidth() &&
                    y >= b.getY() && y <= b.getY() + b.getHeight()) {
                    insideBuilding = true;
                    break;
                }
                // Track nearest building for association (rooftop AI)
            ...
            if (Math.hypot(x - safeZone.getX(), y - safeZone.getY()) < safeZone.getRadius() + 80) {
                continue;
            }

            // Reject if too close to the boat at the moment of spawn
            if (boat != null && Math.hypot(x - boat.getX(), y - boat.getY()) < 120) {
                continue;
            }
            ...
            for (Survivor s : survivors) {
                if (!s.isRescued() && Math.hypot(x - s.getX(), y - s.getY()) < 70) {
            ...
            // Valid location → create survivor
            String name = namePool[random.nextInt(namePool.length)] + " #" + (totalSurvivorsInLevel + spawned + 1);
            Survivor newSurvivor = new Survivor(x, y, name, nearestBuilding);
        ...
        // Feedback
        if (spawned > 0) {
            addFloatingText("WAVE " + waveNumber + ": +" + spawned + " SURVIVORS!", 
                            boat.getX(), boat.getY() - 90, "#facc15");
            statusMessage = "Wave " + waveNumber + " arrived! +" + spawned + " people. Next wave in 30s.";
    ...
    public void update(InputHandler input, double deltaSeconds) {
        ...
        if (survivorSpawnTimer >= SPAWN_INTERVAL) {
            survivorSpawnTimer = 0.0;
            // Random count every wave (example ranges you mentioned: 10, 24, 13 …)
            // We use a wide random range: 8 – 28 people each wave
        ...
        if (timeRemaining <= 0) {
            timeRemaining = 0;
            // With infinite generation, victory = you delivered every survivor that ever appeared
        ...
        // Smooth Hard-Wall Sliding Collision Resolution against Buildings & Trees (NO SLOWDOWN!)
        ...
        // Update Aimed Survivor for Rope Rescue
        updateAimedSurvivor();

        // Update Active Rope Animation
        if (activeRopeAnimation.isActive()) {
            ...
            if (!activeRopeAnimation.isActive() && activeRopeAnimation.getTargetSurvivor() != null) {
                // Rope pull complete! Add to cargo
                ...
                if (!target.isRescued() && boat.canAddCargo()) {
                    ...
                    addFloatingText("ROPE RESCUED " + target.getName() + "!", crew.getWorldX(boat), crew.getWorldY(boat) - 30, "#33CCFF");
                    statusMessage = "Rescued survivor with rope lasso! Deliver to Helipad!";
                }
            }
        }
        ...
        for (Creature creature : creatures) {
            ...
            if (creature.isActive() && creature.getDangerLevel() > 0.8) {
        ...
        // Smooth Push-Out for Road Obstacles (No slowdown when passing obstacles)
        ...
        if (safeZone.isBoatInside(boat) && boat.getCargoCount() > 0) {
            ...
            if (totalDelivered >= totalSurvivorsInLevel) {
                // Temporary clear – next wave may still come
    ...
    private void updateAimedSurvivor() {
        ...
        double closestDist = 450; // Extended rope throw aiming range
        aimedSurvivor = null;
    ...
    private void handleCrewInteraction() {
        ...
        // 1. Try Aim & Throw Rope Rescue to Aimed Survivor
    ...
    private void makeRandomSurvivorScream() {
        ...
        // How many survivors scream this time (1 to 3)
        ...
        for (int i = 0; i < screamCount; i++) {
            ...
            // Bright red / orange so it stands out
    ...
    // Extra getter so HUD can show next wave countdown if you want
