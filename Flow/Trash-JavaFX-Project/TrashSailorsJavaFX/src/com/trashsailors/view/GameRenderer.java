...
public class GameRenderer {
    ...
    public void render(GameEngine engine) {
        ...
        // 2. Draw Immovable IUT Buildings, Mosque, Halls & OIC Tree Park
        ...
        // 9. Draw Aim Line & Target Crosshair to Aimed Survivor
    ...
    private void drawIUTBuilding(Building b) {
        ...
        if ("MOSQUE".equals(b.getType())) {
            ...
            for (double tx = b.getX() + 45; tx < b.getX() + b.getWidth() - 35; tx += 85) {
                for (double ty = b.getY() + 45; ty < b.getY() + b.getHeight() - 35; ty += 85) {
                    gc.setFill(Color.web("#166534"));
                    gc.fillOval(tx - 35, ty - 35, 70, 70);
                    gc.setFill(Color.web("#22c55e"));
                    gc.fillOval(tx - 22, ty - 22, 44, 44);
                }
            }
    ...
    private void drawIUTLabelBadge(Building b) {
        ...
        double textWidth = b.getNameLabel().length() * 8.0;
    ...
    private void drawAimLine(Crew crew, Boat boat, Survivor target) {
        ...
        // Pulsing Golden Aim Line from Crew to Survivor
    ...
    private void drawTrashSailorsRaft(Boat boat) {
        ...
        gc.setFill(Color.web("#3d2514"));
        gc.fillRoundRect(-bw / 2, -bh / 2, bw, bh, 14, 14);

        Color[] plankColors = new Color[] {
                Color.web("#78431e"), Color.web("#5c3316"), Color.web("#925429"), Color.web("#693917")
        };
        double px = -bw / 2 + 4;
        for (int i = 0; i < 6; i++) {
            double pWidth = (bw - 8) / 6;
            gc.setFill(plankColors[i % plankColors.length]);
            gc.fillRect(px, -bh / 2 + 4, pWidth - 2, bh - 8);

            gc.setStroke(Color.web("#261408", 0.6));
            gc.setLineWidth(1.5);
            gc.strokeLine(px + 4, -bh / 2 + 8, px + 4, bh / 2 - 8);

            px += pWidth;
        }

        gc.setFill(Color.web("#475569"));
        gc.fillRect(-bw / 2, -bh / 2, 14, 14);
        gc.fillRect(bw / 2 - 14, -bh / 2, 14, 14);
        gc.fillRect(-bw / 2, bh / 2 - 14, 14, 14);
        gc.fillRect(bw / 2 - 14, bh / 2 - 14, 14, 14);

        gc.setStroke(Color.web("#84cc16", 0.85));
        gc.setLineWidth(4);
        gc.strokeLine(-bw / 3, -bh / 4, -bw / 6, bh / 4);
        gc.strokeLine(-bw / 6, -bh / 4, -bw / 3, bh / 4);

        gc.setFill(Color.web("#523018"));
        gc.fillRect(-6, -6, 12, 12);
        gc.setStroke(Color.web("#ca8a04"));
        gc.setLineWidth(2);
        gc.strokeLine(-12, -2, 12, -2);

        gc.setFill(Color.web("#fef08a"));
        gc.fillOval(-5, -16, 10, 10);
        gc.setStroke(Color.web("#facc15", 0.6));
        gc.setLineWidth(2);
        gc.strokeOval(-8, -19, 16, 16);

        gc.setFill(Color.web("#1e293b"));
        gc.fillRoundRect(-bw / 2 + 6, -14, 20, 28, 6, 6);
        gc.setFill(Color.web("#dc2626"));
        gc.fillRect(-bw / 2 + 8, -10, 16, 8);

        gc.setFill(Color.web("#475569"));
        gc.fillOval(-bw / 2 + 2, -10, 6, 6);
        gc.fillOval(-bw / 2 + 2, 4, 6, 6);

        gc.setFill(Color.web("#334155"));
        gc.fillOval(0, -bh / 2 - 4, 14, 10);
        gc.fillOval(0, bh / 2 - 6, 14, 10);

        gc.setFill(Color.web("#38bdf8"));
        gc.fillOval(-bw / 4, -6, 16, 16);
        gc.setFill(Color.web("#eab308"));
        gc.fillOval(-bw / 4 - 4, -12, 24, 10);
        gc.setStroke(Color.web("#020617"));
        gc.setLineWidth(1.5);
        gc.strokeOval(-bw / 4 + 2, -4, 5, 5);
        gc.strokeOval(-bw / 4 + 8, -4, 5, 5);

        gc.setFont(Font.font("Segoe UI", FontWeight.BOLD, 11));
        gc.setFill(Color.web("#facc15"));
        gc.fillText("P1", -bw / 4 + 2, -15);
        ...
        for (int i = 0; i < boat.getMaxCargoCapacity(); i++) {
            ...
            if (i < boat.getCargoCount()) {
                gc.setFill(Color.web("#00ff88"));
                gc.fillOval(slotX + 2.5, slotY + 3, 8, 8);
                gc.fillRect(slotX + 3.5, slotY + 12, 6, 12);
            }
        }
    ...
    private void drawHUD(GameEngine engine, double w, double h) {
        ...
        if (engine.isLowTimerActive()) {
            gc.setFill(Color.web("#ff0055"));
            gc.fillText("⏰ TIME: " + timeStr + " (SWIM PHASE!)", 40, 56);
        } else {
            gc.setFill(Color.web("#38bdf8"));
            gc.fillText("⏰ TIME: " + timeStr, 40, 56);
        }
        ...
        gc.setFill(Color.web("#f8fafc"));
        gc.fillText("CARGO: " + cargo + "/" + maxCargo, 300, 56);

        gc.setFill(Color.web("#00ff88"));
        gc.fillText("EVACUATED: " + engine.getTotalDelivered() + "/" + engine.getTotalSurvivorsInLevel(), 420, 56);

        gc.setFill(Color.web("#facc15"));
        gc.fillText("SCORE: " + engine.getScore(), 640, 56);

        // Next wave countdown
        int nextWaveSec = (int) Math.ceil(engine.getNextWaveIn());
        gc.setFill(Color.web("#f97316"));
        gc.fillText("WAVE " + engine.getWaveNumber() + " | NEXT: " + nextWaveSec + "s", 780, 56);
        ...
        gc.setFill(Color.web("#00ff88"));
        gc.fillText("⚡ INTERACT / ROPE THROW: [SPACE] or [R]", 430, h - 28);

        gc.setFill(Color.web("#38bdf8"));
        gc.fillText("🔍 ZOOM: Scroll / [Q]/[E]", 750, h - 28);
    }
