...
public class GameEngine {
    ...
    public void update(InputHandler input, double deltaSeconds) {
        ...
        if (!isHelicopter) {
            for (Building b : buildings) {
                if ("FIELD".equals(b.getType())) {
                    continue; // Sports field is open ground
                }

                if ("PARK_TREES".equals(b.getType())) {
                    // Forest Section (OIC Tree Park)
                    if (boat.getVehicleType() == Boat.VehicleType.ROVER) {
                        // Only the 6x6 Land Buggy can drive inside the forest!
                        if (b.intersects(bx, by, bw, bh)) {
                            statusMessage = "🌲 6×6 ROVER: Traversal active in OIC Tree Park! Rescuing forest survivors!";
                        }
                        continue; // Rover can drive freely through the forest
                    } else {
                        // Boat CANNOT enter the forest! Solid tree collision blocks the boat
                        if (b.intersects(bx, by, bw, bh)) {
                            ...
                            bx = boat.getX();
                            by = boat.getY();
                            statusMessage = "⛔ SHALLOW FOREST & DENSE TREES! Only the 6×6 Rover can enter the forest!";
                        }
                        continue;
                    }
                }
    ...
    private void updateAimedTargets() {
        ...
        // 1. Check closest survivor
        for (Survivor s : survivors) {
            if (!s.isRescued()) {
                if (isHeli || !s.isRooftopTrapped()) {
    ...
    private void handleCrewInteraction() {
        ...
        // Check if player in boat/rover is near a rooftop survivor without a helicopter
        if (!isHeli) {
            for (Survivor s : survivors) {
                if (!s.isRescued() && s.isRooftopTrapped()) {
                    double dist = Math.hypot(crewWorldX - s.getX(), crewWorldY - s.getY());
                    if (dist < 320) {
