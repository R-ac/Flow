...
public class GameRenderer {
    ...
    private void drawHelicopterMainRotor(double bw, double bh) {
        ...
        // Dynamic Spinning 4-Blade Carbon Fiber Rotor Assembly (Procedural Real-Time Rotation)
        ...
        // Central Titanium Rotor Mast Hub & Swashplate
        gc.setFill(Color.web("#0f172a"));
    ...
    private void drawRescueBoat(Boat boat) {
        ...
        // 4. Sleek Hydrodynamic Deep-V Hull (Reinforced White Perimeter, Safety Red/Orange Deck)
        // Outer White Reinforced Hull Perimeter Rim
        gc.setFill(Color.web("#f8fafc"));
        ...
        gc.setStroke(Color.web("#0f172a"));
        gc.setLineWidth(2);
        gc.strokePolygon(hullOuterX, hullOuterY, 5);

        // Blue Sponson / Rub-Rail Trim
        gc.setFill(Color.web("#0284c7"));
        double[] sponsonX = { -bw / 2 + 6, bw / 4 - 2, bw / 2 - 4, bw / 4 - 2, -bw / 2 + 6 };
        double[] sponsonY = { -bh / 2 + 2.5, -bh / 2 + 2.5, 0, bh / 2 - 2.5, bh / 2 - 2.5 };
        gc.fillPolygon(sponsonX, sponsonY, 5);

        // Warm Teak Wood Foredeck Planks
        gc.setFill(Color.web("#92400e"));
        double[] bowWoodX = { bw / 8, bw / 4, bw / 2 - 6, bw / 4, bw / 8 };
        double[] bowWoodY = { -bh / 2 + 5, -bh / 2 + 5, 0, bh / 2 - 5, bh / 2 - 5 };
        gc.fillPolygon(bowWoodX, bowWoodY, 5);

        // Wood Plank Lines on Bow
        gc.setStroke(Color.web("#78350f", 0.7));
        gc.setLineWidth(1);
        for (double py = -bh / 2 + 10; py <= bh / 2 - 10; py += 6) {
            gc.strokeLine(bw / 8, py, bw / 2 - Math.abs(py) * 1.8, py);
        }

        // Bow Anchor Cleat & Fairlead (Chrome Marine Hardware)
        double cleatX = bw / 3 + 4;
        gc.setFill(Color.web("#cbd5e1"));
        gc.fillRoundRect(cleatX - 6, -3, 12, 6, 2, 2);
        gc.setStroke(Color.web("#475569"));
        gc.setLineWidth(1);
        gc.strokeRoundRect(cleatX - 6, -3, 12, 6, 2, 2);

        // Main Cockpit & Rescue Cabin Deck Hull (Deep Emergency Red)
        gc.setFill(Color.web("#b91c1c"));
        gc.fillRect(-bw / 2 + 8, -bh / 2 + 6, bw * 0.62, bh - 12);
        gc.setStroke(Color.web("#7f1d1d"));
        gc.setLineWidth(1.5);
        gc.strokeRect(-bw / 2 + 8, -bh / 2 + 6, bw * 0.62, bh - 12);

        // Stainless Steel Safety Handrails along Gunwales
        gc.setStroke(Color.web("#cbd5e1"));
        gc.setLineWidth(2.5);
        gc.strokeLine(-bw / 2 + 10, -bh / 2 + 4, bw / 8, -bh / 2 + 4);
        gc.strokeLine(-bw / 2 + 10, bh / 2 - 4, bw / 8, bh / 2 - 4);

        // 6 Side-Mounted Emergency Flotation Life Rings (3 per side)
        double[] ringXOffsets = { -bw / 3, -bw / 10, bw / 10 };
        for (double rx : ringXOffsets) {
            // Port Side Life Ring
            drawLifeRing(rx, -bh / 2 + 3);
            // Starboard Side Life Ring
            drawLifeRing(rx, bh / 2 - 3);
        }

        // 5. Bow Deck Equipment - Coiled Yellow Rescue Ropes
        gc.setStroke(Color.web("#eab308"));
        gc.setLineWidth(2);
        gc.strokeOval(bw / 5, -bh / 2 + 10, 10, 6);
        gc.strokeOval(bw / 5, bh / 2 - 16, 10, 6);
        gc.strokeOval(-bw / 2 + 16, -bh / 2 + 10, 8, 6);
        gc.strokeOval(-bw / 2 + 16, bh / 2 - 16, 8, 6);

        // 6. Cockpit Console & Curved Aerodynamic Glass Windshield
        double consoleX = bw / 12;
        gc.setFill(Color.web("#0f172a"));
        gc.fillRoundRect(consoleX, -bh / 2 + 7, 8, bh - 14, 4, 4);

        // Curved Windshield Glass (Tinted Cyan)
        gc.setFill(Color.web("#38bdf8", 0.75));
        double[] glassX = { consoleX + 2, consoleX + 9, consoleX + 9, consoleX + 2 };
        double[] glassY = { -bh / 2 + 8, -bh / 2 + 12, bh / 2 - 12, bh / 2 - 8 };
        gc.fillPolygon(glassX, glassY, 4);

        // Pilot P1 at Helm Console
        gc.setFill(Color.web("#38bdf8"));
        gc.fillOval(consoleX - 8, -6, 11, 11);
        gc.setFill(Color.web("#facc15"));
        gc.fillOval(consoleX - 10, -8, 13, 7); // captain's cap

        gc.setFont(Font.font("Segoe UI", FontWeight.BOLD, 10));
        gc.setFill(Color.web("#facc15"));
        gc.fillText("P1", consoleX - 10, -bh / 2 + 3);

        // 7. Extended Passenger Rescue Compartment & Seating Rows
        double cabinStartX = -bw / 2 + 18;
        double cabinW = (consoleX - 10) - cabinStartX;
        double cabinH = bh - 18;
        double cabinY = -cabinH / 2;
        ...
        for (int i = 0; i < maxCapacity; i++) {
            ...
            // Empty Rescue Seat Slot (Brown Leather Cushion)
            gc.setFill(Color.web("#78350f"));
            gc.fillRoundRect(seatX, seatY, seatSlotW - 2, 8, 2, 2);
            gc.setStroke(Color.web("#f8fafc", 0.5));
            gc.setLineWidth(1);
            gc.strokeRoundRect(seatX, seatY, seatSlotW - 2, 8, 2, 2);
            ...
            if (i < boat.getCargoSlots().size()) {
                ...
                if (cType == Boat.CargoType.SURVIVOR) {
                    ...
                    gc.setFill(Color.web("#ea580c"));
                    gc.fillRoundRect(seatX + 0.5, seatY, seatSlotW - 3, 8, 2, 2);
                    ...
                    gc.setStroke(Color.web("#fef08a"));
                    gc.setLineWidth(1);
                    gc.strokeLine(seatX + 2, seatY + 2, seatX + seatSlotW - 4, seatY + 6);
                } else if (cType == Boat.CargoType.METAL) {
                    // Metal Parts in Seat
                    gc.setFill(Color.web("#475569"));
                    gc.fillRoundRect(seatX + 0.5, seatY, seatSlotW - 3, 8, 2, 2);
                    gc.setFill(Color.web("#cbd5e1"));
                    gc.fillRect(seatX + 2, seatY + 2, seatSlotW - 5, 4);
                }
            }
        }

        // 8. Deployable Rescue Boarding Ladders (Port & Starboard)
        gc.setFill(Color.web("#0f172a"));
        gc.fillRect(-bw / 6, -bh / 2 - 4, 16, 4);
        gc.fillRect(-bw / 6, bh / 2, 16, 4);
        gc.setStroke(Color.web("#facc15"));
        gc.setLineWidth(1.5);
        gc.strokeLine(-bw / 6 + 4, -bh / 2 - 4, -bw / 6 + 4, -bh / 2);
        gc.strokeLine(-bw / 6 + 8, -bh / 2 - 4, -bw / 6 + 8, -bh / 2);
        gc.strokeLine(-bw / 6 + 12, -bh / 2 - 4, -bw / 6 + 12, -bh / 2);
        gc.strokeLine(-bw / 6 + 4, bh / 2, -bw / 6 + 4, bh / 2 + 4);
        gc.strokeLine(-bw / 6 + 8, bh / 2, -bw / 6 + 8, bh / 2 + 4);
        gc.strokeLine(-bw / 6 + 12, bh / 2, -bw / 6 + 12, bh / 2 + 4);

        // 9. Stern Roll-Bar Arch with Emergency Red & Blue Strobe Lights
        double rollBarX = -bw / 2 + 10;
        gc.setStroke(Color.web("#cbd5e1"));
        gc.setLineWidth(3);
        gc.strokeLine(rollBarX, -bh / 2 + 4, rollBarX, bh / 2 - 4);

        // Flashing Emergency Beacon Strobes (Red / Blue)
        boolean strobe = ((int)(animTimer * 10) % 2 == 0);
        gc.setFill(strobe ? Color.web("#ef4444") : Color.web("#38bdf8"));
        gc.fillOval(rollBarX - 3, -bh / 2 + 6, 7, 7);
        gc.setFill(!strobe ? Color.web("#ef4444") : Color.web("#38bdf8"));
        gc.fillOval(rollBarX - 3, bh / 2 - 13, 7, 7);

        gc.restore();
    }

    private void drawLifeRing(double cx, double cy) {
        gc.setFill(Color.web("#ea580c"));
        ...
        gc.setStroke(Color.web("#ffffff"));
        gc.setLineWidth(1.2);
        gc.strokeOval(cx - 4.5, cy - 4.5, 9, 9);
    }
    ...
    private void drawSurvivor(Survivor s, boolean isLowTimer) {
        if (s.isRooftopTrapped()) {
            ...
            // 0. Rooftop Contact Ground Shadow
            gc.setFill(Color.web("#020617", 0.45));
            gc.fillOval(rx - 14, ry + 2, 28, 10);
            ...
            gc.fillRect(rx - 6, ry - 1, 4, 5);
            gc.fillRect(rx + 2, ry - 1, 4, 5);

            // 2. Torso / Shoulders & Safety Vest (Top-Down Bird's-Eye View)
            gc.setFill(Color.web("#ea580c")); // Rescue Orange Vest
            gc.fillOval(rx - 11, ry - 7, 22, 14);
            gc.setStroke(Color.web("#c2410c"));
            gc.setLineWidth(1.2);
            gc.strokeOval(rx - 11, ry - 7, 22, 14);

            // Vest Harness Straps
            gc.setStroke(Color.web("#0f172a", 0.6));
            gc.setLineWidth(1);
            gc.strokeLine(rx - 4, ry - 6, rx - 4, ry + 6);
            gc.strokeLine(rx + 4, ry - 6, rx + 4, ry + 6);
            ...
            // Standard Floodwater Survivor (Bobbing in water with lifebuoy & animated character)
            ...
            gc.setLineWidth(1.5);
            gc.strokeOval(s.getX() - 22, sy - 14, 44, 28);

            // Flotation Ring / Lifebuoy (Safety Orange + White Reflective Straps)
            gc.setFill(Color.web("#ea580c"));
            gc.fillOval(s.getX() - 17, sy - 13, 34, 26);
            gc.setFill(Color.web("#0284c7")); // Inner Water
            gc.fillOval(s.getX() - 10, sy - 8, 20, 16);

            // White Straps on Buoy
            gc.setFill(Color.web("#ffffff"));
            gc.fillRect(s.getX() - 16, sy - 3, 5, 6);
            gc.fillRect(s.getX() + 11, sy - 3, 5, 6);
            gc.fillRect(s.getX() - 3, sy - 13, 6, 5);
            gc.fillRect(s.getX() - 3, sy + 8, 6, 5);

            // Student Torso & Green Shirt
            gc.setFill(Color.web("#15803d"));
            gc.fillRoundRect(s.getX() - 8, sy - 7, 16, 14, 4, 4);

            // Head & Hair
            gc.setFill(Color.web("#fed7aa"));
            gc.fillOval(s.getX() - 6.5, sy - 11, 13, 13);
            gc.setFill(Color.web("#451a03"));
            gc.fillOval(s.getX() - 7, sy - 13, 14, 9);

            // Animated Waving Arms
            double waveArm = Math.sin(animTimer * 6) * 6;
            gc.setStroke(Color.web("#fed7aa"));
            gc.setLineWidth(2.5);
            gc.strokeLine(s.getX() - 7, sy - 3, s.getX() - 15, sy - 13 + waveArm);
            gc.strokeLine(s.getX() + 7, sy - 3, s.getX() + 15, sy - 13 - waveArm);
            gc.setFill(Color.web("#fed7aa"));
            gc.fillOval(s.getX() - 17, sy - 15 + waveArm, 4.5, 4.5);
            gc.fillOval(s.getX() + 13, sy - 15 - waveArm, 4.5, 4.5);

            gc.setFont(Font.font("Segoe UI", FontWeight.BOLD, 12));
            gc.setFill(Color.web("#ffffff"));
            gc.fillText(s.getName(), s.getX() - 24, sy - 22);
